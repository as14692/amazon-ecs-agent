...
const (
	TestDaemonName       = "TestDaemon"
	TestImageRef         = "TestImage"
	TestImageTag         = "testTag"
	TestOtherVolumeID    = "otherTestMount"
	TestHealthString     = "testHealth"
...
func TestCreateDaemonTask(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.testName, func(t *testing.T) {
			...
			assert.Equal(t, fmt.Sprintf("arn:::::/%s-", c.testDaemonName), resultDaemonTask.Arn[:20], "Task Arn prefix should match Image Name ")
