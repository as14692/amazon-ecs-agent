...
import (
	...
	mock_api "github.com/as14692/amazon-ecs-agent/ecs-agent/api/mocks"
	rolecredentials "github.com/as14692/amazon-ecs-agent/ecs-agent/credentials"
	...
	mock_retry "github.com/as14692/amazon-ecs-agent/ecs-agent/utils/retry/mock"
	"github.com/as14692/amazon-ecs-agent/ecs-agent/wsclient"
	mock_wsclient "github.com/as14692/amazon-ecs-agent/ecs-agent/wsclient/mock"
...
func TestSessionReconnectsOnDiscoverPollEndpointError(t *testing.T) {
	...
	gomock.InOrder(
		...
		discoverEndpointClient.EXPECT().DiscoverPollEndpoint(gomock.Any()).Return("", fmt.Errorf("oops")),
		// Second invocation returns a success.
		discoverEndpointClient.EXPECT().DiscoverPollEndpoint(gomock.Any()).Return(acsURL, nil),
	)
	acsSession := session{
		containerInstanceARN:   testconst.ContainerInstanceARN,
		discoverEndpointClient: discoverEndpointClient,
...
func TestSessionDoesntLeakGoroutines(t *testing.T) {
	...
	timesConnected := 0
	discoverEndpointClient.EXPECT().DiscoverPollEndpoint(testconst.ContainerInstanceARN).Return(fakeServer.URL, nil).
